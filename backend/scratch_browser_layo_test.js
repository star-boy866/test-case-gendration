...
(async () => {
    ...
    // 6. Select OPR188-LAYO-01 specifically
    console.log('6. Selecting OPR188-LAYO-01...');
    const layoBtn = await page.$('tr:has-text("OPR188-LAYO-01"), td:has-text("OPR188-LAYO-01"), span:has-text("OPR188-LAYO-01")');
    if (layoBtn) {
        console.log('Found OPR188-LAYO-01, clicking it...');
        await layoBtn.click();
        await page.waitForTimeout(2500);
    } else {
        console.log('Searching all rows for LAYO...');
        const allTrs = await page.$$('table tbody tr');
        for (const tr of allTrs) {
            const text = await tr.innerText();
            if (text.includes('OPR188-LAYO-01') || text.includes('LAYO')) {
                console.log('Clicking matched LAYO row:', text.slice(0, 50));
                await tr.click();
                await page.waitForTimeout(2500);
                break;
            }
    ...
    await page.screenshot({ path: 'test_layo_preview_card.png' });
    console.log('Saved test_layo_preview_card.png');
    ...
    if (fullSizeBtn) {
        ...
        await page.screenshot({ path: 'test_layo_modal_full.png' });
        console.log('Saved test_layo_modal_full.png');
    }

    fs.writeFileSync('browser_console_output_layo.json', JSON.stringify(consoleLogs, null, 2));
