...
async def upload_and_generate(
    ...
    try:
        ...
        try:
            ...
            if run.started_at:
                try:
                    st = run.started_at.replace(tzinfo=timezone.utc) if run.started_at.tzinfo is None else run.started_at
                    gen_duration = int((datetime.now(timezone.utc) - st).total_seconds() * 1000)
                except Exception:
                    pass
            gen_meta = GenerationMetadata(
                ...
                input_metadata={"report_id": run.report_id, "file_name": file.filename},
                output_metadata={"scenarios_count": len(pipeline_result.test_suite.test_cases), "coverage": run.coverage_percentage},
                validation_status="PASSED",
            )
            db.add(gen_meta)

            # 3. Persist Generated Scenarios and Baseline Versions (v1: AI_GENERATED)
            for tc in pipeline_result.test_suite.test_cases:
                is_prv = "PRV" in (tc.report_id or "") or "PRV" in tc.test_case_id
                is_nd = "ND" in (tc.report_id or "") or "OPR" in (tc.report_id or "")
                e_method = "Scheduled" if (is_prv or is_nd) else "On Request"
                e_tool = "IWA" if is_prv else ("UC4" if is_nd else "Cognos Portal")

                gen_scen = GeneratedScenario(
                    run_id=run.id,
                    test_case_id=tc.test_case_id,
                    scenario_name=tc.test_case_title or tc.test_case_id,
                    methodology=tc.category,
                    section=tc.source_section,
                    target_field=tc.source_column,
                    risk_level=tc.priority.value if hasattr(tc.priority, 'value') else str(tc.priority),
                ...
                scen_ver = ScenarioVersion(
                    ...
                    content_json={
                        ...
                        "expected_result": tc.expected_result,
                        "validation_sql": getattr(tc, "validation_sql", None),
                        "execution_method": e_method,
                        "execution_tool": e_tool,
                    },
                    source="AI_GENERATED",
                    changed_by="AI Pipeline",
                    change_reason="Initial baseline scenario generated by AI pipeline",
                )
                db.add(scen_ver)
            ...
            log_audit_event(
                ...
                run_id=run.id,
                details={"report_id": run.report_id, "file_name": file.filename, "file_hash": file_hash},
            ...
            log_audit_event(
                ...
                actor_username=current_user.username,
                actor_role=current_user.role,
                action="RUN_GENERATION_COMPLETED",
                resource_type="RUN",
                resource_id=str(run.id),
                run_id=run.id,
                details={"report_id": run.report_id, "scenario_count": len(pipeline_result.test_suite.test_cases)},
                actor_user_id=current_user.id,
            )
            db.commit()
            log_upload_lifecycle(
...
def review_test_case(
    ...
    # ── Governance, Feedback & Scenario Version Persistence ─────────────────
    try:
        gen_scen = (
            db.query(GeneratedScenario)
            .filter(GeneratedScenario.run_id == run_id, GeneratedScenario.test_case_id == test_case_id)
            .first()
        )
        if not gen_scen:
            gen_scen = GeneratedScenario(
                run_id=run_id,
                test_case_id=test_case_id,
                scenario_name=tc.test_case_title or test_case_id,
                methodology=tc.category,
                section=tc.source_section,
                target_field=tc.source_column,
                risk_level=tc.priority,
                description=tc.objective,
                status=tc.review_status or "GENERATED",
                created_by=current_user.username,
            )
            db.add(gen_scen)
            db.flush()
        else:
            gen_scen.status = tc.review_status or gen_scen.status

        # 1. Version snapshot on content change
        if action in ("UPDATE_SCENARIO", "CREATE_REVISION"):
            src = "ADMIN_EDIT" if (current_user.role or "").lower() == "admin" else "TESTER_CORRECTION"
            scen_ver = ScenarioVersion(
                scenario_id=gen_scen.id,
                test_case_id=test_case_id,
                version_number=int(tc.version or 1),
                content_json=_serialize_test_case_model(tc, run_id),
                source=src,
                changed_by=current_user.username,
                change_reason=summary if 'summary' in locals() else f"Action: {action}",
            )
            db.add(scen_ver)

        # 2. Feedback record
        fb_type = "APPROVED" if action == "APPROVE" else ("REJECTED" if action == "REJECT" else ("CORRECTED" if action in ("UPDATE_SCENARIO", "CREATE_REVISION") else "NEEDS_REVIEW"))
        fb_record = ScenarioFeedback(
            scenario_id=gen_scen.id,
            user_id=current_user.id,
            username=current_user.username,
            feedback_type=fb_type,
            comment=payload.review_comments or payload.issue_comment or (summary if 'summary' in locals() else action),
            changed_fields=diffs if 'diffs' in locals() and diffs else None,
        )
        db.add(fb_record)

        # 3. Learning candidate selection on approval
        if action == "APPROVE":
            existing_candidate = db.query(LearningCandidate).filter(
                LearningCandidate.test_case_id == test_case_id
            ).first()
            if not existing_candidate:
                candidate = LearningCandidate(
                    scenario_id=gen_scen.id,
                    test_case_id=test_case_id,
                    source_version_id=int(tc.version or 1),
                    quality_status="QUALIFIED",
                    approval_status="APPROVED",
                    evaluation_score=1.0,
                    selected_for_learning=True,
                    selected_at=now,
                    selected_by=current_user.username,
                )
                db.add(candidate)
            else:
                existing_candidate.approval_status = "APPROVED"
                existing_candidate.selected_for_learning = True
                existing_candidate.selected_at = now
                existing_candidate.selected_by = current_user.username

        # 4. Audit Event
        audit_action_map = {
            "APPROVE": "SCENARIO_APPROVED",
            "REJECT": "SCENARIO_REJECTED",
            "FLAG_ISSUE": "SCENARIO_FLAGGED_FOR_REVIEW",
            "UPDATE_SCENARIO": "SCENARIO_CORRECTED",
            "CREATE_REVISION": "SCENARIO_REVISION_CREATED",
        }
        log_audit_event(
            db=db,
            actor_username=current_user.username,
            actor_role=current_user.role,
            action=audit_action_map.get(action, "SCENARIO_UPDATED"),
        ...
        logger.warning(f"Failed to record scenario review governance data: {g_err}")
