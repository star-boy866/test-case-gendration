...
(async () => {
    ...
    // 6. Select OPR188-LABE-01 specifically
    console.log('6. Selecting OPR188-LABE-01...');
    const labeBtn = await page.$('tr:has-text("OPR188-LABE-01"), td:has-text("OPR188-LABE-01"), span:has-text("OPR188-LABE-01")');
    if (labeBtn) {
        console.log('Found OPR188-LABE-01, clicking it...');
        await labeBtn.click();
        await page.waitForTimeout(2500);
    } else {
        console.log('Searching all rows for LABE...');
        const allTrs = await page.$$('table tbody tr');
        for (const tr of allTrs) {
            const text = await tr.innerText();
            if (text.includes('OPR188-LABE-01') || text.includes('LABE')) {
                console.log('Clicking matched LABE row:', text.slice(0, 50));
                await tr.click();
                await page.waitForTimeout(2500);
                break;
            }
    ...
    await page.screenshot({ path: 'test_labe_preview_card.png' });
    console.log('Saved test_labe_preview_card.png');
    ...
    if (fullSizeBtn) {
        ...
        await page.screenshot({ path: 'test_labe_modal_full.png' });
        console.log('Saved test_labe_modal_full.png');
    }

    fs.writeFileSync('browser_console_output_labe.json', JSON.stringify(consoleLogs, null, 2));
