...
(async () => {
    ...
    const page8 = pages[7];
    const tables = await page8.$$('table');
    if (tables.length > 0) {
        await tables[0].screenshot({
            path: 'd:/test-case-gendration/healthcare-nl-testgen/backend/scratch/page8_table1_spec.png'
        });
        console.log('Saved page8_table1_spec.png successfully');
    }
    await browser.close();
})();
