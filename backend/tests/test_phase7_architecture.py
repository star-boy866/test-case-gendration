...
def _mock_rd(report_id="RPT-TEST-001", title="Test Report"):
    return ReportDefinition(
        ...
        report_fields=[ReportField(field_name="Col A", business_label="Col A", source_table="T", source_column="C")]
    )
...
def test_pattern_applicability_and_limits():
    ...
    reqs = [
        ...
        CognosRequirement(requirement_id="R6", category=RequirementCategory.SPECIAL_PROCESSING, requirement_text="No data"),
        CognosRequirement(requirement_id="R7", category=RequirementCategory.COLUMN_FORMAT, requirement_text="Date format"),
    ...
    applicability_report = discover_applicable_patterns(reqs)
    ...
    patterns_no_special_report = discover_applicable_patterns(reqs_no_special)
    patterns_no_special = patterns_no_special_report.generated
...
def test_many_to_one_mapping_and_id_preservation():
    ...
    label_cases = [c for c in cases if c.category == "Label Validation"]
...
def test_actual_values_and_no_blanks():
    ...
    cases = generate_all_test_cases(rd, req_set)
    label_case = [c for c in cases if c.category == "Label Validation"][0]
...
def test_methodology_applicability_report_type_safety():
    from app.cognos.rules.scenario_patterns import discover_applicable_patterns, MethodologyApplicabilityReport
    
    reqs = [
        CognosRequirement(requirement_id="R1", category=RequirementCategory.COLUMN_LABEL, requirement_text="Column label")
    ]
    
    report = discover_applicable_patterns(reqs)
    
    assert isinstance(report, MethodologyApplicabilityReport)
    
    patterns = report.generated
