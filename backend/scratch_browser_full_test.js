...
(async () => {
    ...
    console.log('1. Navigating directly to /cognos...');
    ...
    // 6. Select Report Header scenario (OPR188-RHDR-01)
    console.log('6. Selecting Report Header scenario...');
    const rhdrRow = await page.$('tr:has-text("RHDR"), tr:has-text("Report Header"), div:has-text("RHDR")');
    if (rhdrRow) {
        console.log('Clicking RHDR row...');
        await rhdrRow.click();
        await page.waitForTimeout(2500);
    }

    // 7. Inspect DSD Evidence Card Preview
    console.log('7. Waiting for DSD Evidence preview to load...');
    await page.waitForTimeout(4000);
    await page.screenshot({ path: 'test_phase13c9_preview.png' });
    console.log('Saved test_phase13c9_preview.png');
    ...
    if (fullSizeBtn) {
        ...
        await page.screenshot({ path: 'test_phase13c9_modal.png' });
        console.log('Saved test_phase13c9_modal.png');
    } else {
        console.log('Open Full Size button not found!');
    }

    fs.writeFileSync('browser_console_output.json', JSON.stringify(consoleLogs, null, 2));
