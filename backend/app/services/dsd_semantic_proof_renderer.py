...
class DSDSemanticProofRenderer:
    """
    Phase 10.8J: Generic DSD Semantic Proof Renderer
    ...
    def render(
        ...
        try:
            ...
            # page_number = 3
            # snapshot_path = jobs/{run_id}/evidence/{evidence_id}.png
            # source_text = report layout semantic content
            ...
            return EvidenceReference(
                ...
                source_text="report layout semantic content" if section_name == "Report Layout" else f"{section_name} semantic content"
            )
                
        except Exception as e:
            logger.warning(f"DSDSemanticProofRenderer generation failed for {target.test_case_id}: {e}")
            return None
    ...
    def _extract_semantic_data(self, target: EvidenceTarget, dsd: NhMmisDsd):
        ...
        if methodology in ("LABEL_VALIDATION", "LAYOUT_VALIDATION"):
            ...
            if dsd.report_specification and len(dsd.report_specification) > 0:
                source_page = dsd.report_specification[0].source_page
            elif dsd.layout:
                source_page = dsd.layout.source_page
            ...
            for idx, row in enumerate(valid_rows):
                ...
                if row.business_label and row.business_label.strip().lower() in target.target_labels:
            ...
            for idx, s in enumerate(valid_sorts):
                ...
                if s.sort_by:
                    s_lower = s.sort_by.strip().lower()
                    if s_lower in target.target_labels or (target.source_column and target.source_column in s_lower):
                        highlights.append(idx)
            ...
            valid_rows = [r for r in dsd.report_specification if r.business_label and r.business_label.strip()]
            for idx, row in enumerate(valid_rows):
                proc_str = (row.processing_rules or "").lower()
                lbl_lower = (row.business_label or "").strip().lower()
                if "date" in proc_str or "mm/" in proc_str or lbl_lower in target.target_labels:
                    rows.append([row.business_label, row.processing_rules])
                    if lbl_lower in target.target_labels or (target.source_column and target.source_column in lbl_lower):
            ...
            for idx, cb in enumerate(valid_cbs):
                ...
                if cb.control_break and target.source_column and target.source_column in cb.control_break.strip().lower():
                    highlights.append(idx)
    ...
    def _draw_evidence_card(
        ...
        total_val = "N/A"
        if dsd.totals and len(dsd.totals) > 0:
            total_val = dsd.totals[0].total
        ...
        draw.text((250, y), f"Department = {dept}", font=self.font_header, fill=(50, 50, 50))
        draw.text((500, y), f"Report Title = {rtitle}", font=self.font_header, fill=(50, 50, 50))
        if total_val != "N/A":
            draw.text((900, y), f"Total = {total_val}", font=self.font_header, fill=(50, 50, 50))
        y += 40
        ...
        for i in range(len(headers)):
            ...
            for row in rows:
                if i < len(row):
                    w = draw.textlength(row[i], font=self.font_body)
                    if w > max_w: max_w = w
